...
def build_system_message(data: ChatIn, harness: Optional[PromptHarness] = None) -> str:
    ...
    return (
        ...
        "For cross-app automations, use open_app, inspect_screen, wait_for_text, tap_target, tap_text, tap_bounds, type_text, clear_text, scroll, swipe, press_back, and press_home steps. Prefer open_app with packageName from the installed apps list, then inspect_screen for unknown screens or wait_for_text before tap/type steps. Prefer stable selectors from screen inspection in this order: viewId, contentDescription, exact text, className plus occurrence. Add timeoutMillis for slow screens, maxNodes for bounded inspections, and settleMillis after app transitions. Cross-app UI control requires the user to enable Aura's Accessibility Service, so avoid direct sends, purchases, deletes, posts, payments, or irreversible actions unless the user explicitly asks and the flow includes a checkpoint. "
                            ...
                            "actions": {
                                ...
                                "items": {
                                    ...
                                    "properties": {
                                        ...
                                        "metadata": {
                                            "type": "object",
                                            "description": "String metadata for executors. ETA actions can use destinationLatitude, destinationLongitude, travelMode, averageSpeedKph, and needsEta=true. Cross-app actions use packageName/appQuery, text, targetText, contentDescription, viewId, className, occurrence, partialMatch, timeoutMillis, settleMillis, maxNodes, direction, or gesture bounds/points.",
                                            ...
                                            "properties": {
                                                ...
                                                "action": {
                                                    ...
                                                    "properties": {
                                                        ...
                                                        "metadata": {
                                                            "type": "object",
                                                            "description": "String metadata. Cross-app actions use packageName/appQuery, text, targetText, contentDescription, viewId, className, occurrence, partialMatch, timeoutMillis, settleMillis, maxNodes, direction, or gesture bounds/points.",
                                                            "additionalProperties": {"type": "string"},
                                                        },
                                                    },
                                                    "required": ["type", "requireConfirmation"],
